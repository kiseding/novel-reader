...
export class FsshuSource implements SiteSource {
  ...
  async downloadPlan(bookId: string): Promise<BookDetail> {
    ...
    $("a").each((_, a) => {
      ...
      if (!m) return;
      chapters.push({ id: m[1], title: cleanText($(a).text()), url: absolutizeURL(BASE, href), order: 0 });
    });

    return { site: this.key, bookId, title, author, description, coverUrl, sourceUrl: `${BASE}${PREFIX}/${bookId}/`, chapters: chapters.map((ch, i) => ({ ...ch, order: i + 1 })) };
  }
